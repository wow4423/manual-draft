...
function screenMarkup(value){
 const text=String(value||'화면 미수집');
 const mission=text.match(/^\[퀘스트\]\s*(.*?)\s*\(미션명\s*:\s*(.*?)\)$/);
 if(!mission)return E(text);
 return '<span class="screen-kind"><span class="screen-quest-tag">퀘스트</span><span>'+E(mission[1])+'</span></span><span class="screen-mission">'+E(mission[2])+'</span>';
}
...
function render(){
 ...
 setMarkup($('deviceRows'),filtered.length?sortFleet(filtered).map(d=>{
  ...
  return '<article class="fleet-device '+(online?'online':'offline')+(attention?' needs-check':'')+'" role="listitem"><button class="fleet-device-open" data-manage-device="'+E(d.id)+'" aria-label="'+E(d.name)+' 상태 보기"><span class="fleet-card-head"><span class="fleet-name">'+connectionIndicator(d.raw)+'<strong>'+E(d.name)+'</strong></span>'+batteryGauge(d.raw.battery,d.raw)+'</span><span class="fleet-screen"><span class="fleet-screen-caption">현재 화면</span><strong>'+screenMarkup(online?d.screen:'전원 미상')+'</strong><span class="fleet-health"><span class="fleet-health-label">'+E(status)+'</span></span></span></button><div class="fleet-card-actions"><span class="fleet-floor">'+E(d.storeName.replace(/^주렁주렁\s*/,''))+' / '+floorText(d.floor)+'</span>'+(window.StaticDemo?.retireDevice?'<button class="fleet-retire" data-retire-device="'+E(d.id)+'" aria-label="'+E(d.name)+' 삭제"><svg viewBox="0 0 20 20" aria-hidden="true"><path d="M3 5h14M7 5V3h6v2M5 5l1 12h8l1-12M8 8v6M12 8v6"/></svg>삭제</button>':'')+'</div></article>';
...
$('loginForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;$('loginSubmit').disabled=true;$('connectionError').textContent='';$('loginStatus').textContent='';try{const result=await api('/session/login',{method:'POST',headers:{'Content-Type':'application/json','X-Concept-Request':'1'},body:JSON.stringify({username:form.elements.username.value.trim(),password:form.elements.password.value})});form.elements.password.value='';user=result.user;await loadData()}catch(err){$('connectionError').textContent=err.message||'로그인하지 못했습니다. 다시 시도해 주세요.';}finally{form.elements.password.value='';$('loginSubmit').disabled=false}};
...
function syncPinPopover(){
 ...
 setMarkup(popup,'<div class="pin-popover-head"><div class="pin-title"><strong>'+E(d.name)+'</strong>'+connectionIndicator(d.raw)+'</div><div class="pin-header-power">'+batteryGauge(d.raw.battery,d.raw)+'</div></div><div class="pin-screen"><span>'+'현재 화면'+'</span><b>'+screenMarkup(Number(d.raw.online)===1?d.screen:'전원 미상')+'</b>'+'</div><section class="pin-connections" aria-label="연결 장치">'+(Number(d.raw.online)!==1?'<h3><small>마지막 수신 기준</small></h3>':'')+'<ul class="pin-hardware">'+[['QR 리더',d.raw.qr_status],['카메라',d.raw.camera_status],['프린터',d.raw.printer_status]].map(([name,raw])=>{const state=!raw||raw==='unknown'?'unknown':['ok','error','no_paper'].includes(raw)?'on':'off';return '<li><span>'+name+'</span><b class="connection-badge '+state+'"'+(state==='unknown'?' title="미수집" aria-label="미수집"':'')+'><i></i>'+(state==='unknown'?'—':state.toUpperCase())+'</b></li>'}).join('')+'</ul></section>'+'<details class="pin-extra-checks"><summary>설정 및 동작 확인</summary>'+deviceExampleLinks(d)+'</details>'+recentDeviceEvents(d)+'<div class="pin-popover-foot"><span>최근 신호 수신 시간</span><time title="'+E(seenText(d.raw.last_seen_at))+'">'+E(seenText(d.raw.last_seen_at).match(/\d{2}:\d{2}:\d{2}/)?.[0]||'기록 없음')+'</time></div>');
...
function renderManagedWorkspace(){
 ...
 '<div class="fd-layout"><div class="fd-main"><section class="fd-screen '+(!online?'muted':'')+'"><span class="fd-section-label">'+(retired?'마지막 화면':'현재 화면')+'</span><strong>' +screenMarkup(retired?(d.raw.current_screen_label||'화면 기록 없음'):online?d.screen:'전원 미상')+'</strong><div class="fd-screen-foot">'+(check?'<span class="fd-guidance"><i></i>'+E(check)+'</span>':retired?'<span>모니터링 종료</span>':'')+'</div></section>'+
